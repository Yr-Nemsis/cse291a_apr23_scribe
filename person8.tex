...
The answer is \textbf{B}. Gradient checkpointing is fundamentally a technique for activation memory only. It works by discarding intermediate activations after the forward pass and recomputing them on demand during the backward pass. Model weights, by contrast, must remain both the forward and the backward computation, as there is no possible way to recompute them during the backward pass, so the same trick cannot be applied to them. The other three statements each describe a correct property of the technique: it does reduce peak GPU memory (A); the choice of where to place the checkpoints directly determines the memory–recomputation trade-off (C); and the underlying reason it is safe to discard activations in the first place is precisely that they are only consumed once the backward pass reaches the corresponding operator (D).
